...
void UrCommunication::run()
{
    ...
    while (keepalive_)
    {
        ...
        {
            // reconnect
            print_warning("Secondary port: No connection. Is controller crashed? Will try to reconnect in 10 "
                          "seconds...");
            sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
            if (sec_sockfd_ < 0)
            {
            ...
            while (keepalive_ && !connected_)
            {
                ...
                if (flag_ < 0)
                {
                    print_error("Error re-connecting to port 30002. Is controller started? Will try to "
