...
void UrRealtimeCommunication::addCommandToQueue(std::string inp)
{
    ...
    if (connected_)
        ...
        print_error("Could not send command \"" + inp + "\". The robot is not connected! Command is "
                                                        "discarded");
}

void UrRealtimeCommunication::setSpeed(double q0, double q1, double q2, double q3, double q4, double q5,
                                       double acc)
{
    char cmd[1024];
...
void UrRealtimeCommunication::run()
{
    ...
    while (keepalive_)
    {
        ...
        {
            // reconnect
            print_warning("Realtime port: No connection. Is controller crashed? Will try to reconnect in 10 "
                          "seconds...");
            sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
            if (sockfd_ < 0)
            {
            ...
            while (keepalive_ && !connected_)
            {
                ...
                if (flag_ < 0)
                {
                    print_error("Error re-connecting to RT port 30003. Is controller started? Will try to "
